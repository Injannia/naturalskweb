...
**Инструментарий:** все UI-проверки и Playwright-тесты в этой фазе пишутся и прогоняются через skill `playwright-cli` (snapshot + refs, навигация, клики, ввод). Это касается как интерактивной верификации фич во время разработки, так и финальных `tests/*.spec.ts`.
